...
for (const p of [wsA, wsB, wsEmpty, wsOutside]) mkdirSync(p, { recursive: true })

function seed(path, rows) {
  const db = new MemoryDb(memoryDbPath(path, DIR))
...
// 会话足迹：alpha 里给 wsA 的 fact 记一条"注入+写过"痕迹
// 注意：按内容挑，不能按 list()[0] —— 同一毫秒插入的多行 created_at 相同，
// ORDER BY created_at DESC 的并列顺序不保证（实测会取到那条 archived fact，
// 于是"元数据齐全/星图会话边"两项随机失败）。
mkdirSync(join(wsA, DIR, 'sessions'), { recursive: true })
const factRow = new MemoryDb(memoryDbPath(wsA, DIR))
...
factRow.close()
writeFileSync(
  join(wsA, DIR, 'sessions', 'session-abc12345.json'),
...
async function call(path, { method = 'GET', headers = {} } = {}) {
  ...
  api.handler({ method, url: path, headers }, res)
...
console.log('— 白名单与安全 —')
{
  ...
  // 关键红线：对没有记忆库的工作区调接口，绝不建库
  const before = existsSync(memoryDbPath(wsEmpty, DIR))
  const r5 = await call('/meow-memory/api/memories?workspace=' + encodeURIComponent(wsEmpty))
  const after = existsSync(memoryDbPath(wsEmpty, DIR))
  check('无库工作区返回 no-db（404）', r5.status === 404 && r5.json?.error?.code === 'no-db')
  check('★ 只读：绝不给别的工作区建库', before === false && after === false)
  const r6 = await call('/meow-memory/api/workspaces')
  const emptySummary = r6.json.data.workspaces.find((w) => w.path === wsEmpty)
  check('无库工作区仍在列表里（hasDb=false）', emptySummary !== undefined && emptySummary.hasDb === false)
}
...
console.log('— 工作区列表 / 总览 —')
{
  ...
  check('alpha 摘要：总数 9', a?.total === 9, String(a?.total))
  check('alpha 摘要：层级分布', a?.counts.project === 4 && a?.counts.fact === 2 && a?.counts.lesson === 1)
  check('alpha 摘要：项目清单', JSON.stringify(a?.projects) === JSON.stringify(['alpha']))
  ...
  check('overview KPI：归档计数', d.kpi.archived === 1 && d.kpi.withDb === 2)
...
console.log('— 星图 —')
{
  ...
  check('星图：记忆节点带 level/workspace', d.nodes.some((n) => n.type === 'memory' && n.level === 'fact' && n.workspace === wsA))
...
{
  const repo = new ViewerRepository(DIR)
  ...
  check('只读 reader 可读', reader !== undefined && reader.counts().fact === 2)
  check('revision 稳定', reader.revision() === reader.revision())
  const rows = reader.listAll()
  const graph = buildGraph(
    [{ workspace: wsA, title: 'alpha', memories: rows, footprints: reader.sessionsFootprint(DIR) }],
