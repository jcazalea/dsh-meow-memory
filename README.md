...
- **会话级记忆开关（client 端，v0.28.0；模型侧零痕迹 v0.32.0）**：composer 输入框旁（发送按钮前）的**「记忆」拨动开关**，绿点=启用 / 灰点=禁用，每会话独立、持久保存。启用：正常注入/检索/生成；**禁用：本会话不注入、不命中、不反思、不自动 dream，`memory_*` 工具返回禁用提示**，且 v0.32.0 起**模型上下文零记忆痕迹**——系统提示词里的记忆手册、`memory_*` 工具定义按会话裁剪（`system-prompt/assemble` waterfall），会话中途禁用后此前注入的长期记忆/命中块也从模型上下文剔除（会话记录本身不动）。禁用时 composer 上方显示提示条「本会话记忆已禁用：不注入 · 不检索 · 不生成（点「记忆」可恢复）」；子代理继承父会话判定（父禁用 → 子代理同样不可见/被拦）。查看器面板与设置页不受影响（用户显式动作）。
